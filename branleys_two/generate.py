...
def memegen(mainImg, text):
    ...
    maxletters = (width // fwidth) * 3  # adjustment because font size is weird
    ...
    imText = Image.new('RGB', (width, textHeight))

    draw = ImageDraw.Draw(imText)

    memetext = "\n".join(lines)
    draw.text((fontsize // 15, fontsize // 15), memetext, (255, 255, 255), font)

    total_h = height + imText.height  # total height of text + image

    # make new image with size of text space + old image
    new_im = Image.new('RGB', (width, total_h))
